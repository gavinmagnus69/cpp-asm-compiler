...
public:
    void print(){
        std::cout << lexeme << " with type " << toke_type_to_string(type) << '\n';
    }
...
private:
    ...
    static bool is_char(const std::string& lexeme){
        return lexeme.front() == '\'' && lexeme.back() == '\'';
    }

    static bool is_integer(const std::string& lexeme){
    ...
    static bool is_double(const std::string& lexeme){
    ...
    static std::string toke_type_to_string(token_type type){
        switch (type)
        {
